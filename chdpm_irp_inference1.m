...
function [ K_list, L_list, CHDPM] = chdpm_irp_inference1( A, D, V, AD, Nd, dv_d_list, dv_v_list, alpha_0, alpha_a, alpha_d, gam, Max_iteration )
    ...
    for dn = 1 : length(dv_d_list)
        ...
        a_d = find(AD(:,d) > 0);
                ...
                if t_selected > Td(d)
                    ...
                    [a_selected, o_selected] = select_option(data_vec, [0 0], a_d, A_d, V, AD,...
                                                    lambda, varphi, eta, theta, OaC, Oa, KC, K, alpha_a, alpha_0, gam);
...
function [Oa, K, lambda, varphi, eta, theta, OaC, KC] = Update_varphi(D, A, V, AD, dv_d_list, dv_v_list, Ad, ...
    ...
    for d = 1 : D
        ...
        for t = 1 : Td(d)
            ...
            [a_selected, o_selected] = select_option(data_vec, ao_old, a_d, A_d, V, AD,...
                lambda, varphi, eta, theta, OaC, Oa, KC, K, alpha_a, alpha_0, gam);
...
function t_selected = select_table(d, v, T_d, a_d, A_d, V, K, Oa, ...
    ...
    for t = 1 : T_d        
        ...
        p(t)       = log(TdC(d, t) +eps) + log(theta(k_t, v) + eps);        
    end

    p(T_d + 1)  =  log(alpha_d+eps) + log(Likehd_dv(v, a_d, A_d, V, K, Oa, OaC, KC, eta, theta, alpha_a, alpha_0, gam) + eps);
    p           =  1 ./ sum(exp(repmat(p, [T_d+1 1]) - repmat(p', [1 T_d+1])), 2);
    t_selected  =  find(mnrnd(1, p) == 1);
end


function [a_selected, o_selected] = select_option(data_vec, ao_old, a_d, A_d, V, AD,...
    lambda, varphi, eta, theta, OaC, Oa, KC, K, alpha_a, alpha_0, gam)
    ...
    k_list = zeros(1, Oall + A_d);
    
    k_max_a = zeros(1, K);
    k_max_p = ones(1, K) * log(eps);
    ...
    for aidx = 1 : A_d
        ...
        for o = 1 : Oa(a)    
            ...
            k_maxp(k_ao) = log(exp(k_maxp(k_ao)) + exp(p_list(num)) + eps);
        ...
        idxa          = find(k_maxp > k_max_p);
        if ~isempty(idxa)
            k_max_p(idxa) = k_maxp(idxa);
            k_max_a(idxa) = a;
        end
    end
    ...
    for k = 1 : K       
        a = k_max_a(k);     
        if a > 0
            idx_nak         = find(a_list ~= a & k_list == k);
            p_list(idx_nak) = log(eps); 
        end
    end
...
function Likehd = Likehd_dv(v, a_d, A_d, V, K, Oa, OaC, KC, ...
    ...
    for aidx = 1 : A_d
        ...
        for o = 1 : Oa(a)  
            k_ao         = eta(a, o); 
            p_list(num)  = OaC(a, o) / (O_a+alpha_a);            
            a_list(num)  = a;
            k_list(num)  = k_ao;            
            k_maxp(k_ao) = k_maxp(k_ao) + p_list(num);            
            num          = num + 1;                     
        end
        
        p_list(num)   = alpha_a/(O_a+alpha_a);
        a_list(num)   = a;
        k_list(num)   = K+1;  
        num           = num + 1;
    ...
    idx_k    = find(k_list > 0 & k_list <= K);    
    Likehd   = Likehd + sum( p(idx_k) * theta(k_list(idx_k), v) );
    
    idx_k1   = find(k_list == K+1);    
    Likehd   = Likehd + sum(p(idx_k1)) * Likehd_k(v_vec, V, K, KC, theta, alpha_0, gam);
