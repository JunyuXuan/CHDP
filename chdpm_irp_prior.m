...
function [ K_list ] = chdpm_irp_prior( A, D, V, AD, DV, alpha_0, alpha_a, alpha_d, Max_iteration )
        ...
        for v = 1 : V
            ...
            if DV(d, v) > 0
                ...
                if t_selected > Td(d)
                    ...
                    % select option
                    [a_selected, o_selected] = select_option([0 0], a_d, A_d, AD,...
                        lambda, varphi, eta, OaC, Oa, KC, K, alpha_a, alpha_0);
...
function [Oa, K, lambda, varphi, eta, OaC, KC] = Update_varphi2(D, A, AD, Ad, ...
    ...
    for d = 1 : D
        ...
        for t = 1 : Td(d)
            ...
            %% sample an option             
            [a_selected, o_selected] = select_option(ao_old, a_d, A_d, AD,...
                lambda, varphi, eta, OaC, Oa, KC, K, alpha_a, alpha_0);
...
function [Oa, K, lambda, varphi, eta, OaC, KC] = Update_varphi(D, A, AD, Ad, ...
          Td, Oa, K, lambda, varphi, eta, OaC, KC, alpha_0, alpha_a, iter)
       
    for a = 1 : A
       
        dt_list          = find(lambda == a);        
        [d_list, t_list] = ind2sub(size(lambda), dt_list);
        
        for dt = 1 : length(dt_list)
            ...
            %% sample an option             
            [a_selected, o_selected] = select_option(ao_old, a, 1, AD, ...
                lambda, varphi, eta, OaC, Oa, KC, K, alpha_a, alpha_0);
            varphi(d, t)             = o_selected;
...
function [a_selected, o_selected] = select_option(ao_old, a_d, A_d, AD,...
    lambda, varphi, eta, OaC, Oa, KC, K, alpha_a, alpha_0)
