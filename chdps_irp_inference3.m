...
function [ K_list, L_list, CHDPS ] = chdps_irp_inference3( A, D, V, AD, Nd, dv_d_list, dv_v_list, alpha_0, alpha_a, alpha_d, gam, Max_iteration )
...
function t_selected = select_table(d, v, T_d, a_d, A_d, V, K, Oa, ...
    ...
    p_w   = zeros(1, T_d+sum(Oa(a_d))+K+1);  
    p_L   = zeros(1, T_d+sum(Oa(a_d))+K+1);
    p_a   = zeros(1, A_d);
    N_d   = sum(TdC(d, :));
    KCsum = sum(KC);
    num   = 1;
...
%         p_w(num)   = TdC(d, t)/(N_d + alpha_d);
%         p_L(num)   = log(theta(k_t, v) + eps);  
%         num        = num + 1;
%     end  
...
%         for o = 1 : Oa(a) 
%             p_w(num) = (alpha_d/(N_d + alpha_d)) * (OaC(a, o) / ( (O_a+alpha_a)*A_d ) );
%             p_L(num) = log(theta(eta(a,o), v) + eps);
%             num      = num + 1;          
%         end     
        p_w(num:(num+Oa_a-1)) = (alpha_d/(N_d + alpha_d)) * (OaC(a, 1:Oa_a) / ( (O_a+alpha_a)*A_d ) );
        p_L(num:(num+Oa_a-1)) = log(theta(eta(a,1:Oa_a), v) + eps);
        num                   = num + Oa_a;  
            
        p_a(aidx)             = alpha_a / ((O_a + alpha_a)*A_d) ;
        k_weight_tmp          = k_weight_tmp + p_a(aidx);
    end
%     for k = 1 : K
%        p_w(num) = (alpha_d/(N_d + alpha_d)) * k_weight_tmp * KC(k) / ( KCsum+alpha_0 );
%        p_L(num) = log(theta(k, v) + eps);
%        num      = num + 1;
%     end
    p_w(num:(num+K-1)) = (alpha_d/(N_d + alpha_d)) * k_weight_tmp * KC / ( KCsum+alpha_0 );
    p_L(num:(num+K-1)) = log(theta(1:K, v) + eps);
    num                = num + K;
       
    p_w(num)           = (alpha_d/(N_d + alpha_d)) * k_weight_tmp * alpha_0 / ( KCsum+alpha_0 );
    p_L(num)           = -log(V + eps);
    ...
    if t_s <= T_d        
        ...
        a = a_d(find(mnrnd(1, p_a/sum(p_a)) == 1));
...
%     for k = 1 : K
%         Lik(k) = sum(data_vec .* log(theta(k, :) + eps));
%     end  
...
%         for o = 1 : Oa(a)    
% 
%             p_w(num) = OaC(a, o) / ((O_a+alpha_a) * A_d);
%             p_L(num) = Lik(eta(a,o)); 
%             num      = num + 1;
%         end
        p_w(num:(num+Oa_a-1)) = OaC(a, 1:Oa_a) / ((O_a+alpha_a) * A_d);
        p_L(num:(num+Oa_a-1)) = Lik(eta(a, 1:Oa_a)); 
        num                   = num + Oa_a;
...
%     for k = 1 : K
%         p_w(num) = k_weight_tmp * KC(k) / (KCsum + alpha_0);
%         p_L(num) = Lik(k); 
%         num      = num + 1;
%     end
    
    p_w(num:(num+K-1)) = k_weight_tmp * KC / (KCsum + alpha_0);
    p_L(num:(num+K-1)) = Lik(1:K); 
    num                = num + K;    
    p_w(num)           = k_weight_tmp * alpha_0 / (KCsum + alpha_0);
    p_L(num)           = Lik(K+1); 
